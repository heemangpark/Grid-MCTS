import time

import torch

from env.multi_env import maze_env
from mcts_nn.tree_multi import MultiTree
from rl.q_agent_loc import QAgent
from utils.arguments import maze_args
from utils.visualize import vis_route, vis_multi_route_total

if __name__ == "__main__":
    s = time.time()

    for id in range(1):
        n_ag = 2
        env = maze_env(maze_args, n_ag)
        env.size = 30
        env.difficulty = .2
        max_step = 500

        agent = QAgent()
        agent.load_state_dict(torch.load('./sacred/rand_binaryrwd_best.th', 'cuda'))

        _, _ = env.reset()
        tree = MultiTree(env, agent, n_ag)
        tree.grow(max_step=max_step)

        e = time.time()
        print(e - s)

        seq = []
        for i in range(n_ag):
            individual_seq = [s[i] for s in tree.state_seq]
            seq.append(individual_seq)
            vis_route(env.maze[i], individual_seq, env.start_loc[i], env.goal_loc[i],
                      'agent_{}_trial_{}'.format(i + 1, id + 1))

        vis_multi_route_total(env.maze, seq, env.start_loc, env.goal_loc)
